...
class MpesaService
{
    ...
    public function hasLiveConfiguration(): bool
    {
        return
            trim($this->apiKey) !== '' &&
            trim($this->publicKeyStr) !== '' &&
            trim($this->baseUrl) !== '' &&
            trim($this->serviceProviderCode) !== '';
    }
    ...
    public function initiatePayment(string $phoneNumber, float $amount, string $reference)
    {
        ...
        // Use a single normalized reference for both provider fields to keep status queries deterministic.
        $normalizedReference = $this->normalizeProviderReference($reference, 10);
        $formattedAmount = $this->formatAmount($amount);
        $formattedPhone = $this->formatPhoneNumber($phoneNumber); // "25884..."

        $payload = [
            "input_TransactionReference" => $normalizedReference,
            "input_CustomerMSISDN" => $formattedPhone,
            "input_Amount" => $formattedAmount,
            "input_ThirdPartyReference" => $normalizedReference,
        ...
        try {
            Log::info('Mpesa C2B request initiated.', [
                ...
                'third_party_reference' => $normalizedReference,
                'amount' => $formattedAmount,
            ]);

            $response = $this->httpRequest($bearerToken, [
                // C2B often answers near ~64s (INS-9). Keep client timeout above that to capture provider response.
                'timeout' => max(75, $this->timeoutSeconds),
                'connect_timeout' => max(10, $this->connectTimeoutSeconds),
            ])
            ...
            Log::info('Mpesa C2B response received.', [
                ...
                'response_desc' => $response->json('output_ResponseDesc'),
            ]);
            ...
            if ($response->successful()) {
                ...
                if ($this->isSuccessfulResponseCode($responseCode)) {
                    return [
                        'success' => true,
                        'data' => $data,
                        'transaction_id' => $data['output_TransactionID'] ?? $normalizedReference,
                        'third_party_reference' => $normalizedReference,
            ...
            return [
                ...
                'transaction_id' => isset($data['output_TransactionID']) ? (string) $data['output_TransactionID'] : $normalizedReference,
                'third_party_reference' => $normalizedReference,
                'technical_message' => $providerMessage ?? ('Gateway Error: HTTP ' . $response->status()),
            ];
        } catch (ConnectionException $e) {
            $technicalMessage = $e->getMessage();
            Log::error('Mpesa Connection Error: ' . $technicalMessage);
            $isTimeout = $this->isTimeoutLikeTransportError($technicalMessage);

            return [
                'success' => false,
                'message' => $this->humanizeTransportError($technicalMessage, 'payment')
                    ?? $this->defaultUserMessage('payment'),
                'technical_message' => $technicalMessage,
                'response_code' => $isTimeout ? self::RESPONSE_CODE_TIMEOUT : null,
            ];
        } catch (\Exception $e) {
    ...
    private function isTimeoutLikeTransportError(string $technicalMessage): bool
    {
        $message = strtolower($technicalMessage);
    ...
    private function userMessageForResponse(?string $responseCode, ?string $providerMessage, string $context = 'payment'): string
    {
        ...
        $mappedByCode = match ($code) {
            ...
            'INS-21' => 'Parâmetros de pagamento inválidos. Verifica o número M-Pesa e tenta novamente.',
    ...
    private function formatPhoneNumber(string $number): string
    {
        ...
        if (str_starts_with($number, '258258') && strlen($number) === 15) {
            // Guards accidental double country prefix (e.g. 25825884xxxxxxx).
            return substr($number, 3);
        }

        if (str_starts_with($number, '0') && strlen($number) === 10) {
            return '258' . substr($number, 1);
        }
    ...
    private function normalizeProviderReference(string $reference, int $length, string $prefix = ''): string
    {
        $clean = strtoupper((string) preg_replace('/[^A-Z0-9]/i', '', $reference));
        $prefix = strtoupper((string) preg_replace('/[^A-Z0-9]/i', '', $prefix));
        $maxBodyLength = max(1, $length - strlen($prefix));
        $body = substr($clean, 0, $maxBodyLength);

        if ($body === '') {
            $body = strtoupper(substr(uniqid('', true), 0, $maxBodyLength));
            $body = preg_replace('/[^A-Z0-9]/', '', $body) ?: 'REF';
        }

        return substr($prefix . $body, 0, $length);
    }
